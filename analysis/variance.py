...
from Tools import gen_feed_dict_BeRNN
...
def _compute_variance_bymodel(model, sess, rules=None, random_rotation=False):
    ...
    for rule in rules:
        ...
        while currentRule != rule:
            currentBatch = random.choice(AllTasks_list)
            if len(currentBatch.split('_')) == 6:
                currentRule = currentBatch.split('_')[2] + ' ' + currentBatch.split('_')[3]
            else:
                currentRule = currentBatch.split('_')[2]

        # co: It might be better to randomly draw the trials used for training and validation, as there might be more error in the last trials
...
def compute_variance(model_dir, rules=None, random_rotation=False):
    ...
    Args:
        ...
        random_rotation: boolean. If True, rotate the neural activity.
    """
    dirs = valid_model_dirs(model_dir)
    for d in dirs:
