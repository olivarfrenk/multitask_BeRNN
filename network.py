...
class LeakyRNNCell(RNNCell):
    ...
    def __init__(self,
        ...
        if self._w_rec_init == 'diag':
            ...
            w_rec0 = self._w_rec_start*gen_ortho_matrix(n_hidden, rng=self.rng) # co: tools.gen_ortho_matrix
...
class Model(object):
    ...
    def __init__(self,
        ...
        if hp is None:
            hp = load_hp(model_dir)               # co: tools.load_hp(model_dir)
            if hp is None:
                raise ValueError(
    ...
    def _build(self, hp):
        ...
        if hp['l1_weight'] > 0:
            self.cost_reg += hp['l1_weight'] * tf.add_n(                # add_n: element wise sum of tensors
    ...
    def _build_fused(self, hp):
        ...
        # co: Where the error is coming from..
        # if hp['loss_type'] == 'lsq':
        #     self.c_mask = tf.placeholder("float", [None, n_output])         # co: cmask issue
        # else:
        #     # Mask on time
        ...
        if hp['loss_type'] == 'lsq':
            ...
            self.cost_lsq = tf.reduce_mean(
                tf.square((y_shaped - y_hat)))
                # tf.square((y_shaped - y_hat) * self.c_mask))  # co: c_mask issue
        else:
            y_hat = tf.nn.softmax(y_hat_)
            # Cross-entropy loss
            self.cost_lsq = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_shaped, logits=y_hat_))

            # self.cost_lsq = tf.reduce_mean(                   # co: c_mask issue
    ...
    def _build_seperate(self, hp):
        ...
        self.x = tf.placeholder("float", [None, None, n_input])
        self.y = tf.placeholder("float", [None, None, n_output])
        # self.c_mask = tf.placeholder("float", [None, n_output])           # co: c_mask issue
